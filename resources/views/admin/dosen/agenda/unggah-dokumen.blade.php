        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Manajemen Dokumen Akhir</h3>
                </div>
                <div class="card-body">
...
$(document).ready(function() {
    var table = $('#kegiatan-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.dosen.agenda.unggah-dokumen.list') }}",
        columns: [
    ...
    $('#kegiatan-table').on('click', '.download-btn', function() {
        ...
        window.location.href = "{{ route('admin.dosen.agenda.unggah-dokumen.download', ['', '']) }}/" + id + "/" + type;
    });

    // Handle tombol delete
    $('#kegiatan-table').on('click', '.delete-btn', function() {
        var id = $(this).data('id');
        var type = $(this).data('type');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('admin.dosen.agenda.unggah-dokumen.destroy', ['', '']) }}/" + id + "/" + type,
                    type: "DELETE",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    $('#uploadForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('admin.dosen.agenda.unggah-dokumen.store') }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#editForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('admin.dosen.agenda.unggah-dokumen.update') }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
